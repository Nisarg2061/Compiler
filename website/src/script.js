...
function fetchLexer() {
  fetch("http://cb.bynisarg.in/lexer")
...
function fetchSemantic() {
  fetch("http://cb.bynisarg.in/semantic")
    .then((response) => response.json())
    .then((data) => {
      displayOutput(data.output || data.error);
    })
...
function uploadFile() {
  ...
  fetch("http://cb.bynisarg.in/upload", {
    method: "POST",
    body: formData,
  })
    .then((response) => response.json())
